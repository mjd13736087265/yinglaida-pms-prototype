...
PC.reg('/api/push','消息推送', (el)=>{
  ...
  <div class="card"><h3>推送模板</h3>
    ${table([{t:'模板',k:'t'},{t:'场景',k:'s'},{t:'内容预览',k:'p'},{t:'操作',r:r=>`<button class="btn sm ghost" onclick="UI.toast('模板已保存')">编辑</button>`}],[
      {t:'账单生成通知',s:'每月账单日','p':'【区域产业】您{月份}{类型}账单 ¥{金额} 已生成，请于{截止日}前缴纳 →'},
      {t:'欠费催缴提醒',s:'逾期 1/7/15 天','p':'【区域产业】您有{笔数}笔账单已逾期{天数}天，合计 ¥{金额}，请尽快缴纳。'},
      {t:'余额不足预警',s:'表计余额低于阈值','p':'【区域产业】您的{表类型}余额 ¥{余额} 已低于 {阈值} 元，为避免断电请立即充值。'},
      {t:'合同到期提醒',s:'到期前 30/60/90 天','p':'【区域产业】您的{房源}租赁合同将于{到期日}到期，如需续租请联系管家。'}])}
  </div>`;
});
})();
